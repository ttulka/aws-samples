import {AppIdentityProvider, UserInfo} from '../src/app-identity-provider'
import * as fs from 'fs'

describe('Provide Token for Apps Test', function () {
    ...
    it('Token must be provided', async function (done) {
        try {
            const jwt = await tokenProvider.token(testUserId, testPassword, testTenantId, privateKey, fakeIdentityProvider())
    ...
    function fakeIdentityProvider(): AppIdentityProvider {
        return {
